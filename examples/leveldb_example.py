"""
leveldb (https://github.com/google/leveldb) in use as a persistent Graph store.
via the Plyvel LevlDB-Python interface (https://github.com/wbolster/plyvel)

The store is named and referenced as "LevelDB".

Example 1: simple actions

* creating a ConjunctiveGraph using the LevelDB Store
...
* loads multiple graphs downloaded from GitHub into a LevelDB-baked graph stored in the folder gsq_vocabs.
...
def example_1():
    ...
    # Declare we are using a LevelDB Store
    store = plugin.get("LevelDB", Store)(
        identifier=URIRef("rdflib_leveldb_test")
    )
    graph = ConjunctiveGraph(store)
    path = mktemp(prefix="testleveldb")
